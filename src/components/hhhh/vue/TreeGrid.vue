...
<!--		<el-table-column label="序号">
		...
		<el-table-column v-for="(column, index) in columns" :key="column.sortField" :label="column.name">
			<template scope="scope">
				...
				<b v-if="index===0 " :class="scope.row.icon"></b> {{scope.row.name}}
			</template>
		</el-table-column>
	...
	export default {
		...
		methods: {
			...
			toggleIconShow(index, record) {
				let me = this
				if(me.treeStructure && index === 0 && record && record.length > 0) {
					return true
				}
				return false
			},
